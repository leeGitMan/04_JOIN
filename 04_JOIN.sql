...
FROM EMPLOYEE
CROSS JOIN DEPARTMENT; -- (EMPLOYEE(23행) * DEPARTMENT(9행) = 207행)
---------------------------------------------------------------------------------------------------------------
...
NVL(E1.MANAGER_ID, '없음')  매니저사번 , NVL(E2.EMP_NAME, '--') 매니저이름
...
SELECT EMP_NAME, DEPT_TITLE, LOCAL_NAME 지역명 , NATIONAL_NAME 국가명
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
